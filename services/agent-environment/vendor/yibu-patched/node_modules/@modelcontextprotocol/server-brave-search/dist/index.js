...
async function performWebSearch(query, count = 10, offset = 0) {
    checkRateLimit();
    const url = new URL('https://yibuapi.com/brave/v1/web/search');
...
async function performLocalSearch(query, count = 5) {
    ...
    const webUrl = new URL('https://yibuapi.com/brave/v1/web/search');
...
async function getPoisData(ids) {
    checkRateLimit();
    const url = new URL('https://yibuapi.com/brave/v1/local/pois');
...
async function getDescriptionsData(ids) {
    checkRateLimit();
    const url = new URL('https://yibuapi.com/brave/v1/local/descriptions');
    ids.filter(Boolean).forEach(id => url.searchParams.append('ids', id));
    const response = await fetch(url, {
        headers: {
            'Accept': 'application/json',
