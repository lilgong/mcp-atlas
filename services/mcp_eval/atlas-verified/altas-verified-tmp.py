...
def get_function_call(TRAJECTORY):
    func_call_list = []
    trajectory_list = json.loads(TRAJECTORY) if isinstance(TRAJECTORY, str) else TRAJECTORY
    for index, trajectory in enumerate(trajectory_list):
        ...
        if not tool_calls:
            continue
        # Official MCP-Atlas trajectories have role=assistant; current synthetic p3
        # trajectories may only contain tool_calls for server extraction. Support both.
        if trajectory.get("role") not in (None, "assistant"):
            continue
        for in1, item in enumerate(tool_calls):
            if not isinstance(item, dict):
                continue
            function = item.get("function") or {}
            result = ""
            result_index = index + in1 + 1
            if result_index < len(trajectory_list) and isinstance(trajectory_list[result_index], dict):
                result = trajectory_list[result_index].get("content", "")
...
def get_raw_traj_function(trajectory, raw_conversation_function,threshold=0.4):
    ...
    for function in new_raw_function:
        ...
        if not same_name_trajectory_function:
            continue
        # 合成(synthetic)轨迹的 GT tool_call 只有工具名、没有 arguments（p3 只保留 name）。
        # 这种情况无法做参数语义相似度匹配，"工具名一致"即视为调用了正确工具。
        # 官方轨迹带 arguments，gt_args_empty=False，仍走下面的参数相似度逻辑，行为不变。
        if all(not item.get("arguments") for item in same_name_trajectory_function):
            exist_function.append({
                "trajectory_function": handle_function_result(same_name_trajectory_function[0]),
